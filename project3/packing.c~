...
void count_wh(Node **p, int ind)//use postorder to find the width and height of each node
{
  if(p[ind] -> width > 0)
  {
    return;
  }
  count_wh(p, p[ind] -> lcnode - 1);//visit left child
  count_wh(p, p[ind] -> rcnode - 1);//visit right child
  if(p[ind] -> cutline == 'V')
  {
...
void count_xy(Node **p, int ind)//use preorder to find xcoord and ycoord of each node
{
  if(p[ind] -> lcnode < 0)
  {
    return;
  ...
  count_xy(p, p[ind] -> lcnode - 1);//visit left child
  ...
  count_xy(p, p[ind] -> rcnode - 1);//visit right child
